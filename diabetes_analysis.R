...
data <- data.frame(read_excel("F:/datasets/diabetes_data/Diabetes_Classification.xlsx", 
                            col_types = c("skip", "numeric", "numeric", "numeric", "numeric", "numeric", 
...
set.seed(5678)
logistic.spec <- trainControl(method="cv", number=10,
                                    savePredictions="all",
                                    classProbs=F)

logistic.validation <- train(Diabetes ~ Glucose+Age,
                             data=pruned_training_set,
                             method="glm",
                             family="binomial",
                             trControl=logistic.spec)
...
set.seed(5678)
bayes.model <- train(Diabetes ~., data=training_set, method="nb",
...
set.seed(5678)
svm.model1 <- svm(Diabetes ~., data=training_set, 
...
p<-predict(svm.model, newdata=test_set)
confusionMatrix(p, test_set$Diabetes)


# svm model creation and generating predictions on test data
set.seed(5678)
svm.model <- train(Diabetes ~., data=training_set, method="svmLinear",
                   trControl=trainControl("cv", number=10),
                   preProcess=c("center", "scale")
)
svm.model2 <- train(Diabetes ~., data=training_set, method="svmPoly",
                   trControl=trainControl("cv", number=10),
                   preProcess=c("center", "scale"),
                   tuneLength=4
)
...
svm.predictions2 <- predict(svm.model2, test_set)



#model accuracy 
confusionMatrix(svm.predictions2, test_set$Diabetes)
